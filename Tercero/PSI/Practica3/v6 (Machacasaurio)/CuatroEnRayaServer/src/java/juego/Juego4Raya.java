...
public class Juego4Raya implements AIJuego<Tablero, Jugador, Movimiento>{
	...
	public List<Movimiento> posiblesMovimientos(Tablero tablero, Jugador player) {
		...
		for(int columna = 0; columna < 7; columna++){
                    if(tablero.getCelda(5, columna).esVacia()){
	...
	@Override
	public Movimiento esVacio(Tablero t, Jugador j){
    	for(int i = 0; i < 7; i++){
    		if(!t.getCelda(0, i).esVacia()){
    			return null;
    		}
    	}
    	Celda c = new Celda(0, 3);
    	return new Movimiento(c, j);
    }
