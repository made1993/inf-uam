 ...
 * Roberto Garcia Teodoro (roberto.garciat@estudiante.uam.es)
*/
 extern void * WhoseIsThisSocket(int socket);
 extern SSL * getSSL(void * client);
int recibir(int sockfd,char *buf){
	int aux =0;
	void* client=NULL;
	SSL* ssl=NULL;
	if((client= WhoseIsThisSocket(sockfd))== NULL){
		return -1;
	}else{
		
		if((ssl=getSSL(client))!=NULL){
...
/**
 *
 * @brief evalua si se ha realizado la conexion SSL con exito
 ...
 * 	\b #include \b "conexion.h"
 *
 *	\b int \b enviar_datos_SSL \b (\b SSL* ssl, \b const coid * buf\b )
 * 
 * @section descripcion DESCRIPCIÓN
 *	
 ...
 *	
 * @section seealso VER TAMBIÉN
 * \b fijar_contexto_SSL(3), \b inicializar_nivel_SSL(3), \b conectar_canal_seguro_SSL(3), \b aceptar_canal_seguro_SSL(3),
 * \b evaluar_post_connectar_SSL(3), \b enviar_datos_SSL(3), \b  recibir_datos_SSL(3), \b cerrar_canal_SSL(3) \b
 * @section authors AUTOR
 * Mario Valdemaro Garcia Roque (mariov.garcia@estudiante.uam.es)
 * Roberto Garcia Teodoro (roberto.garciat@estudiante.uam.es)
...
/**
 *
 * @brief recibe un mensaje SSL
 * @page recibir_datos_SSL \b recibir_datos_SSL
 *
 * @section SYNOPSIS
 * 	\b #include \b "conexion.h"
 *
 *	\b int \b recibir_datos_SSL \b (\b SSL* ssl, \b void* buf\b )
 * 
 * @section descripcion DESCRIPCIÓN
 *	
 ...
 * 	\b #include \b "conexion.h"
 *
 *	\b void \b cerrar_canal_SSL \b (\b SSL * ssl, \b SSL_CTX* ctx, \b int sockfd\b )
 * 
 * @section descripcion DESCRIPCIÓN
 *	
