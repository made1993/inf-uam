	...
	switch(IRCUser_CommandQuery(msg)){
		case UCYCLE:
			...
			for(i=0;i<numchannels;i++){
				IRC_Join(command, NULL, str[i], NULL,NULL);	
				escribir(sockfd, command);			
			}
	...
	*/while(1){
		...
		switch(command){
			...
			case JOIN:
				printf("JOIN\n");
                                printf("Buffer:%s\n",&buf[pos]);
				IRCParse_Join(&buf[pos], &usr, &ch, &aux2,&aux12);
				pch = strtok(usr,"!");
				if(strcmp(nickName,pch)==0){
					...
					printf("canal  %s\n",ch);
					set_current_page(get_index_page(ch));
			...
			case PING:
				printf("PING\n");
                                IRC_Pong(msg, NULL, get_server(), NULL);
                                printf("se envia:%s\n", msg);
		                escribir(sockfd,msg);
			break;
			case PONG:
