...
(defun navigate-white-hole (state white-holes)
	...
	(if (null white-holes)
		NIL
   (mapcar #'(lambda (x) (make-action 
                          :name 'navigate-white-holes 
                          :origin state  
                          :final (second x) 
                          :cost (third x)))
...
(defun navigate-worm-hole (state worm-holes)
	...
	(if (null worm-holes)
		NIL
   (mapcar #'(lambda (x) (make-action :name 'navigate-worm-holes 
                                      :origin state  
                                      :final (if (equal state (second x)) (first x) (second x))
                                      :cost (third x)))
...
;;;	Inicializa una estructura llamada *galaxy-M35* que representa el problema 
;;;   bajo estudio.
;;;
(setf *galaxy-M35* (make-problem 
                    :states *planets*
...
;;;					*uniform-cost*))	
;;;								;->Caso tipico
;;;		(insert-nodes-strategy '( ) '( ) *uniform-cost*))	;-> NIL
;;;                                                              ;->Caso especial
;;;
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;	Llama a la funcion que realiza busqueda en grafo inicializando previamente las 
;;;   listas de nodos vacios y nodos cerrados.
;;;
;;;	INPUT: 
...
(defun graph-search-aux (problem strategy open-nodes closed-nodes)
  (if (null open-nodes)
      NIL
    (if (funcall(fn-name (problem-fn-goal-test problem)) 
                 (node-state (first open-nodes)) 
                 (fn-lst-args (problem-fn-goal-test problem)))
        (first open-nodes)
...
;;;		Caso especial sería en el que una de las dos listas que recibe fueran
;;;         nulas.
;;;	
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;
;;;	EJEMPLOS:
;;;		(tree-path (a-star-search *galaxy-m35*))	;-> (KENTARES KATRIL DAVION 
;;;                                                  SIRTIS)	;-> Caso tipico
;;;		
...
;;;	INPUT: 
;;;		node: Nodo al que se ha llegado
;;;		states: Lista donde guardar los nombres de los planetas por los que 
;;;         se ha pasado para llegar al nodo
;;;	OUTPUT:
;;;		Lista con los nombres de los planetas por los que se ha pasado para 
;;;         llegar al nodo o NIL.
;;;
...
;;;				;-> (KENTARES AVALON MALLORY KATRIL DAVION SIRTIS)
;;;                              ;-> Caso tipico
;;;
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
