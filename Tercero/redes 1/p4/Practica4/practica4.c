...
uint8_t moduloIP(uint8_t* segmento, uint16_t* pila_protocolos,uint64_t longitud,void *parametros){
	...
	*((Parametros*)parametros)=ipdatos;
	...
	for(i=0;i<npaq;i++){
		...
		if(npaq==i+1){
			aux16=longitud -offset*8;
			aux16=htons(aux16+20);
			memcpy(datagramaaux + 2, &aux16, sizeof(uint16_t));
			aux16=0;
			aux16<<=13;
			aux16+=htons(offset/8);
			memcpy(datagramaaux + 6, &aux16, sizeof(uint16_t));
			memcpy(datagramaaux+20,segmento,sizeof(uint8_t)*(longitud -offset));

			protocolos_registrados[protocolo_inferior](datagramaaux,pila_protocolos,longitud+20,parametros);
		}
		else{
			printf("\t\t\t\t%dº  fragmento\n",i+1 );
			aux16=MTU;
			...
			memcpy(datagramaaux + 6, &aux16, sizeof(uint16_t));
			offset=+MTU;
			memcpy(datagramaaux+20,segmento,sizeof(uint8_t)*MTU);

			protocolos_registrados[protocolo_inferior](datagramaaux,pila_protocolos,offset+20,parametros);
