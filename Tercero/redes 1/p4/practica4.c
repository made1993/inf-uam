...
uint8_t moduloIP(uint8_t* segmento, uint16_t* pila_protocolos,uint64_t longitud,void *parametros){
	...
	uint16_t MTU;
	int npaq,i=0, posflags=0,auxtam=0;
	uint16_t offset=0;
	...
	*((Parametros*)parametros)=ipdatos;
	for(i=0;i<npaq;i++){
		...
		if(i+1==npaq){
			...
			aux16=0;
			aux16<<=13;
			offset=longitud-offset;
			aux16+=htons(offset);//posicion
			memcpy(datagramaaux+posflags,&aux16,sizeof(uint16_t));
			memcpy(datagramaaux+2,&offset,sizeof(uint16_t));
			calcularChecksum(pos, datagramaaux, checksum);
			...
			aux16+=htons(offset);//posicion
			memcpy(datagramaaux+posflags,&aux16,sizeof(uint16_t));
			memcpy(datagramaaux+2,&MTU,sizeof(uint16_t));
		}
		offset+=MTU;
