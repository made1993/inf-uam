...
public abstract class Usuario implements Serializable{
			...
			if(g.getNombre().equals(grupo.getNombre()) && g.getIdGrupo()==grupo.getIdGrupo()){
				if(enviarBuzon){
					for(Usuario u:g.getListaUsuarios()){
						this.enviarMensaje(id,u, mensaje, Calendar.getInstance(), "Mensaje de Grupo: "+g.getNombre(), this);
					}
				}
	...
	public Usuario cargarUsuario(String correo) throws IOException, ClassNotFoundException{
		...
		FileInputStream fis = new FileInputStream("aplicacion"+m.getBarra()+"usuario"+
										m.getBarra()+correo+m.getBarra()+"datos.obj");
	...
	public boolean guardarUsuario() throws IOException{
		MailUam m=new MailUam();
		FileOutputStream fos = new FileOutputStream("aplicacion"+m.getBarra()+"usuario"+
									m.getBarra()+getCorreo()+m.getBarra()+"datos.obj");
