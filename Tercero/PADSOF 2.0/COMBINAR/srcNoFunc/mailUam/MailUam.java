...
public class MailUam {
	...
	public boolean addMensajeGrupo(MensajeGrupo mensaje, Grupo grupo) {
		cargarGrupos();
		return grupo.addMensaje(mensaje);
	}
	...
	public Usuario cargarUsuario(String correo) {
		...
		/*for(Grupo g: usr.getListaGrupos()){
			for(Grupo g2: listaGrupos){
				if(g.getNombre().equals(g2.getNombre())){
					g= g2;
				}
			}
		}*/
	...
	public boolean guardarUsuario(Usuario usuario) {
		...
		/*for(Grupo g: usuario.getListaGrupos()){
			for(Grupo g2: listaGrupos){
				//System.out.println("G es:"+g.getNombre()+"\nG2 es:"+g2.getNombre()+"\n");
				if(g2.getNombre().equals(g.getNombre())){
					//System.out.println("\nHay una correspondencia de nombre\n");
					g2=g;
					//System.out.println(((GrupoEstudio)g2).getListaPreguntas());
					//System.out.println(((GrupoEstudio)g2).getListaPreguntas().get(0).getListaRespuestas());
				}
			}
		}
		System.out.println("se guardan los grupos");
		guardarGrupos();*/
		return true;
	}
		...
		try {
			...
			if (buscarListaGrupos(nombre)) {// TODO revisar
				if (!borrarGrupo(nombre))
					return grupo;// TODO igual excepocion
			}
