...
public class ControlBuscarGrupo implements ActionListener {
	...
	/**
	 * Constructor de ControlBuscarGrupo
	 * 
	 * @param v
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (source.equals(buscarGrupo.getBotonMensajes())) {
		...
		} else if (source.equals(buscarGrupo.getBotonVerPerfil())) {
			...
			for (Component c : buscarGrupo.getPanelResultados().getComponents()) {
				...
				if (check.isSelected()) {
					Grupo g = getModelo().buscarGrupo(
							check.getText().split("]")[1]);
			...
			getModelo().cargarGrupos();
			buscarGrupo.setValores(getModelo().buscarGrupoLista(
					buscarGrupo.getTextBuscarText()));
		}
	}
