...
void new_text (char *msg)
{
	...
	switch(IRCUser_CommandQuery(msg)){
		...
		case UJOIN:/*TODO entra en un canal aunque no le pongas el #, pese a que no lo reconoce como canal*/
			...
			sprintf(nch,"%d",numchannels);
			//IRC_Join(command, NULL, str[0], nch);
			//escribir(sockfd,command);
			printf("%s\n",command );
			for(i=0;i<numchannels;i++){
				IRC_Join(command, NULL, str[i], nch);
				escribir(sockfd,command);
				add_new_page(str[i]);
				set_current_page(get_index_page(str[i]));
				message_text(current_page(),"TE HAS UNIDO");
			}
		break;
		case UPART:
			printf("UPART\n");
			IRCUserParse_Part(msg, &str2);
			
			IRC_Part(command, NULL, *str, str2);/*TODO??*/
		break;
		case ULEAVE:
			printf("ULEAVE\n");
			IRCUserParse_Leave(msg, &str2);
		...
		case UKICK:
			printf("UKICK\n");
			IRCUserParse_Kick(msg, &str2, &usr);
			IRC_Kick(command,NULL, str2, usr, "TU MADRE");
			escribir(sockfd, command);
		break;
		case UTOPIC:
			printf("UTOPIC\n");
			IRCUserParse_Topic(msg,&ch,&str2);
			IRC_Topic(command,NULL, ch,str2);
			escribir(sockfd,command);
		break;
		case UME:
			printf("UME\n");
		...
		case UMODE:
			printf("UMODE\n");
			//IRCUserParse_Mode(msg,&ch,&str2);
			IRC_Mode(command,NULL, ch, str2);
			escribir(sockfd,command);
	...
	while(1){
		...
		switch(command){
			...
			case JOIN:
				...
				gdk_threads_enter();
				printf("2\n");
				message_text(get_index_page(ch),"se ha unido");
				//message_text(get_index_page(ch),pch);
				gdk_threads_leave();
			break;
			case PART:
				printf("PART\n");
			...
			case PRIVMSG:
				...
				//get_index_page(aux2);
				gdk_threads_enter();
				public_text( get_index_page(aux2),aux1,aux3);
				gdk_threads_leave();
			break;
			case NOTICE:
				printf("NOTICE\n");
