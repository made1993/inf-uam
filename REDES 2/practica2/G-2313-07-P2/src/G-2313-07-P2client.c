#include "../includes/G-2313-07-P2client.h"
...
CLIENT* createClient(int socket){
	...
	now = time(0);
	c->lastConnection=malloc(sizeof(localtime(&now)));
	memcpy(c->lastConnection, localtime(&now), sizeof(localtime(&now)));
	return c;
}
...
void setLastConnectionClient(CLIENT* client){
	time_t now;
	//free(client->lastConnection);
	now = time(0);
	client->lastConnection=malloc(sizeof(localtime(&now)));
