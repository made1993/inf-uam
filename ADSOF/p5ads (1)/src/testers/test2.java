...
public class test2 {
	public static void main(String[] args) {
		...
		g.connect(n1, 7, n3);
		g.connect(n2, 1, n3);
		System.out.println("Todo nodo de g conectado con n3? "+g.forAll(n -> n.equals(n3) || n.isConnectedTo(n3))); // true
		System.out.println("Existe exactamente un nodo de g conectado con n2? "+g.one( n -> n.isConnectedTo(n2))); // true
		System.out.println("Existe al menos un nodo de g conectado con n2? "+g.exists( n -> n.isConnectedTo(n2))); // (*) true
		g.exists( n -> n.getValue().equals(89)); // No se cumple: el nodo witness es null
		g.getWitness().ifPresent( w -> System.out.println("Witness 1 = "+g.getWitness().get()));
		g.exists( n -> n.isConnectedTo(n2)); // Se cumple: el nodo witness está definido
		...
		BlackBoxComparator<Integer, Integer> bbc = new BlackBoxComparator<Integer, Integer>()
				.addCriteria( Criteria.Existential, n -> n.isConnectedTo(2))
		...
		System.out.println(cgs); // imprime g (cumple la 1ª propiedad) y luego g1 (cumple la 2º y 3º)
