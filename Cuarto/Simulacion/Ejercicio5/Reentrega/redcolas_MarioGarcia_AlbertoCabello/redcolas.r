...
schednewarrv <- function() {
  ...
  schedevnt(arrvtime,"arrv",list(arrvtime=arrvtime, beginq=arrvtime))
}
...
schednewarrv2 <- function(head) {
  ...
  if(runif(1, 0, 1) > mm1glbls$oldpercent) { #NEW
    ...
    if(length(mm1glbls$newq) == 0) {
      mm1glbls$newq <<- sim$currtime
      srvdonetime <- sim$currtime + timeserver3()
      schedevnt(srvdonetime, "srvdone3", list(arrvtime=head$arrvtime, beginq=sim$currtime))
    }
    else mm1glbls$newq <<- c(mm1glbls$newq, sim$currtime)
  }
  else { #OLD
    type <- "old"
    #print("Generado old")
    ...
    if(length(mm1glbls$oldq) == 0) {
      mm1glbls$oldq <<- sim$currtime
      srvdonetime <- sim$currtime + rexp(1, mm1glbls$srv2rate)
      schedevnt(srvdonetime, "srvdone2", list(arrvtime=head$arrvtime, beginq=sim$currtime))
    }
    else mm1glbls$oldq <<- c(mm1glbls$oldq, sim$currtime)
    #print(type)
  }
...
reactarrival <- function(head) {
  ...
  if (length(mm1glbls$srvq) == 0) {
    ...
    schedevnt(srvdonetime,"srvdone",list(arrvtime=head$arrvtime, beginq=head$beginq))
  } 
...
reactdone1 <- function(head) {
  ...
  mm1glbls$q1time <<- c(mm1glbls$q1time, sim$currtime - head$beginq)
  ...
  # schedule new service
    srvdonetime <- sim$currtime + rnormT(1,mm1glbls$srv1avg, mm1glbls$srv1dsv)
    schedevnt(srvdonetime,"srvdone",list(arrvtime=mm1glbls$srvq[1], beginq=mm1glbls$srvq[1]))
  }
}
#React to srvdone2
reactdone2 <- function(head) {
  ...
  mm1glbls$q2time <<- c(mm1glbls$q2time, sim$currtime - head$beginq)
  ...
  # schedule new service
    srvdonetime <- sim$currtime + rexp(1,mm1glbls$srv2rate)
    schedevnt(srvdonetime,"srvdone2",list(arrvtime=mm1glbls$oldq[1], beginq=mm1glbls$oldq[1]))
  }  
}
#React to srvdone3
reactdone3 <- function(head) {
  ...
  # schedule new service
    srvdonetime <- sim$currtime + timeserver3()
    schedevnt(srvdonetime,"srvdone3",list(arrvtime=mm1glbls$newq[1], beginq=mm1glbls$newq[1]))
  }  

  #Statistics
  mm1glbls$acabados <<- mm1glbls$acabados + 1
  mm1glbls$q3time <<- c(mm1glbls$q3time, sim$currtime - head$beginq)
}
...
graficos1 <- function() {
  ...
  plot(density(mm1glbls$q1time),main="Funcion de densidad tiempo en cola1")
  ...
  plot(density(mm1glbls$q2time),main="Funcion de densidad tiempo en cola3")
  
  plot(mm1glbls$q3size, type='h', main='Evolución de cola en servidor general',xlab='Tiempo', ylab='Tamaño de cola3')
  plot(density(mm1glbls$q3size),main="Funcion de densidad tamaño de cola3")
  plot(density(mm1glbls$q3time),main="Funcion de densidad tiempo en cola3")
...
if (TRUE) {
 ...
 dosim (mm1initglbls, mm1reactevnt, mm1prntrslts, maxsimtime=5000.0, 
     list(arrvrate=.3), dbg=FALSE)
