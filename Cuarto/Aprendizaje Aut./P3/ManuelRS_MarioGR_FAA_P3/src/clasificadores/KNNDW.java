...
public class KNNDW extends Clasificador {
		...
		for(int i=0; i<dataTest.getNumDatos();i++){
			...
			for(int k=0;k<knn;k++){
				if(false==prediccion.containsKey(des[k][1]))
					prediccion.put(des[k][1], 1.0/Math.pow(des[k][0],2));
				else
					prediccion.put(des[k][1], prediccion.get(des[k][1])+1.0/Math.pow(des[k][0],2));
			}
