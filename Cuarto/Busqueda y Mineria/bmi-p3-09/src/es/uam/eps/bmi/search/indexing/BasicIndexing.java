...
public class BasicIndexing implements Index, Serializable{
	...
	private Runtime rt;
	private boolean indiceDisco;
	private boolean guardarIndices;
	
	public BasicIndexing(boolean indiceDisco, boolean guardarIndices) {
		super();
		this.indices= new HashMap<>();
		this.modulos= new HashMap<>();
		this.docIds= new ArrayList<>();
		this.indicesDisco=null;
		this.indiceDisco=indiceDisco;
		this.guardarIndices=guardarIndices;
		this.textDocs= new ArrayList<>();
		this.inputPath="basic";
	...
	public void build2(String inputCollectionPath, String outputIndexPath, TextParser textParser) {
		...
		try {
			...
			while(zs.hasMoreElements()){
				...
				String[] tokens=null;
				if(doc.length()<=255)
					textDocs.get(i).setDoc(doc.substring(0, doc.length()));
				else
					textDocs.get(i).setDoc(doc.substring(0, 255));
		...
		/*
		 * Se calculan los modulos de los documentos
		 * */
		double tf,idf;
		...
		/*
		 * Se guardan los indices a fichero
		 * */
		if(guardarIndices ||(rt.totalMemory()-rt.freeMemory())/ Math.pow(2, 20)>=2048 ){
			time= System.nanoTime();
			
			File fout= new File(outputIndexPath+"/"+inputPath+"/indices/");

			System.out.println("guardando los indices en:"+ outputIndexPath+"/"+inputPath);
			fout.mkdirs();
				
			ObjectOutputStream fos;
			try {
				fos = new ObjectOutputStream(new FileOutputStream(outputIndexPath+"/"+inputPath+"/textDoc"));
			    fos.writeObject(textDocs);
			    fos.close();
			    
			    fos = new ObjectOutputStream(new FileOutputStream(outputIndexPath+"/"+inputPath+"/modulos"));
			    fos.writeObject(modulos);
			    fos.close();

			    fos = new ObjectOutputStream(new FileOutputStream(outputIndexPath+"/"+inputPath+"/docIds"));
			    fos.writeObject(docIds);
			    fos.close();
			    
			    for(String s:indices.keySet()){
					if(s.length()==0 || s.length()>255)
						continue;
					fos=new ObjectOutputStream(new FileOutputStream(outputIndexPath+"/"+inputPath+"/indices/"+s));
	...
	public void build(String inputCollectionPath, String outputIndexPath, TextParser textParser) {
		...
		try {
			time=System.nanoTime();
			/*
			 * Se leen los documentos del zip
			 * 
			 * */
			...
			time=System.nanoTime();
			/*
			 * Se parsean los documentos y se crean los 
			 * TextDocuments
			 * */
	...
	/*public void build2(String inputCollectionPath, String outputIndexPath, TextParser textParser) {
		...
		if(true ||(rt.totalMemory()-rt.freeMemory())/ Math.pow(2, 20)>=2048 ){
			time= System.nanoTime();
	...
	*/
	...
	public void load(String indexPath) {
		try {
			ObjectInputStream ois=new ObjectInputStream(new FileInputStream(indexPath+"/"+inputPath+"/textDoc"));
			textDocs= (List<TextDocument>) ois.readObject();
			ois.close();
			ois=new ObjectInputStream(new FileInputStream(indexPath+"/"+inputPath+"/modulos"));
			modulos=(HashMap<String, Double>) ois.readObject();
			ois.close();
			

			ois=new ObjectInputStream(new FileInputStream(indexPath+"/"+inputPath+"/docIds"));
			docIds= (ArrayList<String>) ois.readObject();
			ois.close();
		...
		File fin= new File(indexPath+"/"+inputPath+"/indices/");
		inputPath=indexPath+"/"+inputPath+"/indices/";
		System.out.println("cargando los indices precontruidos de:"+ inputPath);
	...
	public List<Posting> getTermPostings(String term) {
		if(indicesDisco!=null){
			if(!indicesDisco.contains(term))
				return null;
			try {
				System.out.println("Cargando indice en: "+inputPath+term);
	...
	public static void main (String[] args){
		BasicIndexing i= new BasicIndexing(true , true);
		...
		time=System.nanoTime();
		i.build("/home/alumnos/e282034/Downloads/clueweb-10K/docs.zip", "/home/alumnos/e282034/Downloads/clueweb-10K/index", new HTMLSimpleParser());
		time= System.nanoTime()-time;
